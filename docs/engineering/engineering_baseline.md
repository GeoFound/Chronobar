...
实现前：
- 明确陈述假设。如有不确定，请提问。
- 如存在多种解释，请呈现它们 - 不要默默选择。
- 如存在更简单的方法，请说明。在必要时提出异议。
- 如有不清楚的地方，请停止。命名困惑之处。提问。
...
- 不要添加未请求的"灵活性"或"可配置性"。
- 不要为不可能的场景添加错误处理。
- 如果写了 200 行而可以写成 50 行，请重写。

自问："资深工程师会说这太复杂了吗？"如果是，请简化。
...
强大的成功标准允许独立循环。弱标准（"使其工作"）需要持续澄清。

**这些准则生效的标志：** diff 中不必要的变更更少，因过度复杂导致的重写更少，澄清问题在实现之前而非错误之后出现。
